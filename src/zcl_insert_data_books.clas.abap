CLASS zcl_insert_data_books DEFINITION
  PUBLIC FINAL
  CREATE PUBLIC.
...
CLASS zcl_insert_data_books IMPLEMENTATION.
  METHOD if_oo_adt_classrun~main.
    DATA lt_ACC_CATEG TYPE TABLE OF ztb_acc_categ_jr.
    DATA lt_CATEGO    TYPE TABLE OF ztb_catego_jrc.
    DATA lt_CLIENTES  TYPE TABLE OF ztb_clientes_jrc.
    DATA lt_CLNTS_LIB TYPE TABLE OF ztb_clnts_lib_jr.
    DATA lt_LIBROS    TYPE TABLE OF ztb_libros_jrc.
    ...
    DELETE FROM ztb_catego_jrc.
    INSERT ztb_catego_jrc FROM TABLE @lt_catego.
    out->write( |Categorias { sy-dbcnt }| ).

    " Tabla ztb_clientes_jrc
    lt_CLIENTES = VALUE #( ( id_cliente  = '005638984K'
                             tipo_acceso = '1'
                             nombre      = 'Andrew'
                             apellidos   = 'Roberts'
    ...
    DELETE FROM ztb_clientes_jrc.
    INSERT ztb_clientes_jrc FROM TABLE @lt_clientes.
    out->write( |Clientes { sy-dbcnt }| ).

    " Tabla ztb_libros_jrc
    lt_libros = VALUE #(
        ( id_libro  = '000001'
          bi_categ  = 'A'
        ...
        ( id_libro  = '000072'
          ...
          formato   = 'P'
          url       = 'https://eltinteroeditorial.com/wp-content/uploads/2017/06/c%C3%B3mo-es-el-proceso-de-edici%C3%B3n.jpg' ) ).
    DELETE FROM ztb_libros_jrc.
    INSERT ztb_libros_jrc FROM TABLE @lt_libros.
    out->write( |Libros { sy-dbcnt }| ).

    " Tabla ztb_clnts_lib_jr
    lt_CLNTS_LIB = VALUE #( ( id_cliente = '005638984K' id_libro = '000001' )
    ...
    out->write( |Clientes con Libros { sy-dbcnt }| ).
  ENDMETHOD.
ENDCLASS.
