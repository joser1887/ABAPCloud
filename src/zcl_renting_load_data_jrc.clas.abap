CLASS zcl_renting_load_data_jrc DEFINITION
  PUBLIC FINAL
  CREATE PUBLIC.
...
CLASS ZCL_RENTING_LOAD_DATA_JRC IMPLEMENTATION.


  METHOD if_oo_adt_classrun~main.
    DATA lt_brands    TYPE TABLE OF zrent_brands_jrc.
    DATA lt_cars      TYPE TABLE OF zrent_cars_jrc.
    DATA lt_customers TYPE TABLE OF zrent_client_jrc.
    ...
    lt_brands = VALUE #(
        ...
        ( marca = 'BENTLEY'
          url   = 'https://www.autobild.es/sites/autobild.es/public/styles/main_element/public/dc/fotos/Bentley-Continental_GT-2018-C01.jpg?itok=_kP_edH4' )
        ( marca = 'BUGATTI' url = 'https://wtfonline.mx/wp-content/uploads/2018/12/buggatti.jpg' )
        ( marca = 'FERRARI'
          url   = 'https://api.ferrarinetwork.ferrari.com/v2/network-content/medias/resize/5dd552852cdb32285a785d2e-line-up-ferrari-sf90-stradale?apikey=9QscUiwr5n0NhOuQb463QEKghPrVlpaF&width=800&height=600' )
        ...
        ( marca = 'HONDA'
          url   = 'https://cdn.autobild.es/sites/navi.axelspringer.es/public/styles/480/public/media/image/2017/06/643677-honda-civic-type-r-2017.jpg?itok=pmaP2b_K'  )
        ...
        ( marca = 'SEAT'
          url   = 'https://www.seat.mx/content/countries/mx/seat-website/es/modelos/ibiza/_jcr_content.resizedViewPort.noscale.rangeMiniature.png' )
        ( marca = 'VW'
          url   = 'https://www.vwimg.com/iris/iris?bkgnd=transparent&fabric=BC&paint=0Q0Q&pov=E06,CGN&quality=100&vehicle=2020_BU32MS_2019_10_27&Resp=png&width=510&crop=20,90,450,232' )
    ...
    INSERT zrent_brands_jrc FROM TABLE @lt_brands.
    SELECT * FROM zrent_brands_jrc INTO TABLE @lt_brands.
    out->write( sy-dbcnt ).
    out->write( 'zrent_brands data inserted successfully!' ).
    " CARS--
    lt_cars = VALUE #( und_potencia = 'CV'
                       moneda       = 'USD'
                       alquilado    = 'X'
                       ( matricula   = '1246GSA'
                         marca       = 'BMW'
                       ...
                       ( matricula   = '29348KJ'
                         ...
                         alq_desde   = '20211106'
                         alq_hasta   = '20211206' ) ).
    DELETE FROM zrent_cars_jrc.
    INSERT zrent_cars_jrc FROM TABLE @lt_cars.
    SELECT * FROM zrent_cars_jrc INTO TABLE @lt_cars.
    out->write( sy-dbcnt ).
    ...
    DELETE FROM zrent_client_jrc.
    INSERT zrent_client_jrc FROM TABLE @lt_customers.
    SELECT * FROM zrent_client_jrc INTO TABLE @lt_customers.
    out->write( sy-dbcnt ).
